...
export default function ActionMultiple({ orderDataList, ordertype }) {
  ...
  const onSubmit = (data) => {
    ...
    orderDataList.forEach((parentOrder) => {
      if (!loopExecuted && parentOrder.childrens.length > 0) {
        ...
        createOrderPartialExit(updatedChildOrder);
        loopExecuted = true; // Update flag variable to indicate loop execution
      }
    });
    handleModal();
    reset();
      ...
      <div className="flex gap-1">
        ...
        {ordertype === "order" ? (
          <button
            ...
            onClick={handleModal}
            className="font-semibold invisible text-regular py-2 px-7 inline-block rounded-md bg-black disabled:bg-grey-400"
          >
            Partial Exit
          </button>
        ) : (
