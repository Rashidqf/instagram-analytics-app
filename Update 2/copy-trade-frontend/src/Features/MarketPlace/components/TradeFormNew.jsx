...
export default function TradeFormNew({ marketData = {} }) {
  ...
  const onsubmit = (data) => {
    ...
    if (orderLevels === entryPrice) {
      hitToast("Order level and Entry price can't be same", "info");
    }
    console.log(data);
    ...
    if (tradeType === "order" && !data.amount) {
      hitToast("Order level is required", "error");
    }
    if (!data.percentage) {
      return hitToast("Amount is required", "error");
    }
  ...
  const handleIncrement = (inputName) => {
    const value = parseInt(watch(inputName)) || 0;
    reset({
      [inputName]: value + 1,
    });
  };
      ...
      <form onSubmit={handleSubmit(onsubmit)}>
        <div className="p-3">
          ...
          <div className="pt-1 flex items-end border-b border-b-grey-400 pb-3">
            ...
            <div>
              <p className="text-sm text-center tracking-wide">Stop Loss</p>
              <input
                type="number"
                step="any"
                min="0"
          ...
          <div className={`flex items-end justify-center gap-1 px-1 mt-1`}>
            <div>
              <p className="text-sm text-center tracking-wide">Entry</p>
              <div className="flex justify-center items-center">
                <ArrowDown
                  onClick={() => handleDecrement("entryPrice")}
                  size={25}
                />
                ...
                />
                <ArrowUp
                  onClick={() => handleIncrement("entryPrice")}
                  size={25}
                />
              </div>
            </div>
            ...
            {/* {tradeType === "order" ? (
              <div>
                <p className="text-sm text-center tracking-wide">Order Level</p>
                <input
                  type="number"
                  step="any"
                  {...register("orderLevel")}
