...
import { calAmmountbyU,genLog } from "../order/order.helper";
    ...
    try {
      ...
      await Promise.all(
        accounts.map(async (account) => {
          ...
          const ammount = calAmmountbyU(account.u, stopLoss, entryPrice, side);
          console.log(ammount);
          // const marketData =
          ...
          if (ammount && ammount !== 0) {
            ...
            const newTrade = await db.create({
              table: Trade,
              key: { ...req.body,orderLevel: stopLoss, account: account._id, populate },
            });
...
export const createTradetoOrder =
  ...
  async (req, res) => {
    try {
      ...
      await Promise.all(
        accounts.map(async (account) => {
          ...
          if (ammount && ammount !== 0) {
            ...
            const newTrade = await db.create({
              table: orderSchema,
              key: { ...req.body,toOrder: "toOrder", account: account._id, populate },
            });
            orders.push(newTrade);
          }
        })
      ...
      try {
        const trade = await Trade.findByIdAndUpdate(req.body.id, { status: 'Closed' });
    
        if (trade) {
          console.log(`Trade ${req.body.id} status updated to "Closed" successfully.`);
        } else {
...
export const manageOrder =
  ...
  async (req, res) => {
    try {
      const { status, orderCreated, openPrice, tradeId, id,reformattedData } = req.body;
      console.log(req.body);
